...
def fresnel(theta, lambda_i, n, z, sigma, det_sensitivity = 1, ares = 0, polarization = 0.00, mfc = 1):
    ...
    theta_i = np.zeros((len(theta), len(sigma)+1), dtype=complex)
    theta_i[:, 0] = np.pi/2.- np.array(theta) 
    theta_t = np.zeros((len(theta), len(sigma)+1), dtype=complex)
        
    # interface profile
    s = np.zeros((len(theta), len(sigma)), dtype=complex)
    w = np.zeros((len(theta), len(sigma)), dtype=complex) 
    
    # reflection and transmission
    r_s = np.zeros((len(theta), len(sigma)), dtype=complex)
    r_p = np.zeros((len(theta), len(sigma)), dtype=complex) 
    
    t_s = np.zeros((len(theta), len(sigma)), dtype=complex)
    t_p = np.zeros((len(theta), len(sigma)), dtype=complex) 
    
    mfc_r_s = np.zeros((len(theta), len(sigma)), dtype=complex)
    mfc_r_p = np.zeros((len(theta), len(sigma)), dtype=complex) 
    ...
    net_r_s = np.zeros((len(theta_i), len(sigma)+1),dtype=complex)
    net_r_p = np.zeros((len(theta_i), len(sigma)+1),dtype=complex)
    beta = np.zeros((len(theta_i), len(sigma)),dtype=complex)
    ...
    for k in range(len(lambda_i)): # looping over all wavelengths
        ...
        for i in range(len(sigma)-1, -1, -1):
            ...
            net_r_s[:, i] = (mfc_r_s[:, i] + net_r_s[:, j]*np.exp(complex(0, 2)*beta[:, i]))/(1 + mfc_r_s[:, i]*net_r_s[:, j]*np.exp(complex(0, 2)*beta[:, i]))
            
            net_r_p[:, i] = (mfc_r_p[:, i] + net_r_p[:, j]*np.exp(complex(0, 2)*beta[:, i]))/(1 + mfc_r_p[:, i]*net_r_p[:, j]*np.exp(complex(0, 2)*beta[:, i]))
